...
import type { Book, Category, Status } from "./tipos-libro";
...
/**
 * La vista de la biblioteca. Sigue siendo de cliente —tiene formularios, filtros, capas
 * modales y estado por todas partes—, pero ya NO es quien decide cuándo se leen los libros.
 *
 * `librosIniciales` llega del componente de servidor de page.tsx, ya resueltos. Si viene
 * `null` es que la base falló durante el render: entonces, y solo entonces, esta vista
 * vuelve al camino de antes y los pide ella misma a /api/books.
 */
export default function Biblioteca({librosIniciales}:{librosIniciales:Book[]|null}) {
  const [books,setBooks] = useState<Book[]>(librosIniciales??[]);
  const [loading,setLoading] = useState(librosIniciales===null);
  ...
  useEffect(()=>{
    // Si el servidor ya mandó los libros no hay nada que pedir: repetir la petición aquí
    // volvería a pagar justo la ida y vuelta que este cambio venía a quitar.
  ...
  // En el movil el desplegable de orden hace tambien de filtro de categoria, asi que su
  // rotulo tiene que decir que hay filtrado. En escritorio no hace falta: las categorias
  // siguen siendo pastillas visibles.
  const filtrosActivos=[categoryFilter,filter==="Todos"?null:filter].filter(Boolean);
  const etiquetaMovil=filtrosActivos.length?filtrosActivos.join(" · "):sortLabels[sortOrder];

  return <main>
    {/* `inicio-topbar`: las reglas de movil de esta barra son suyas y de nadie mas.
        Sin esta clase se colaban en la barra de /coleccion, que comparte `.topbar`. */}
    <header className="topbar inicio-topbar">
      ...
      <div className="library-toolbar">
        <div className="sort-control" ref={sortMenuRef}>
          ...
          <button className="sort-trigger" type="button" aria-haspopup="menu" aria-expanded={sortOpen} onClick={()=>setSortOpen(open=>!open)}>
            <b><span className="solo-escritorio">{filter==="Todos"?sortLabels[sortOrder]:filter}</span><span className="solo-movil">{etiquetaMovil}</span></b><i aria-hidden="true">⌄</i>
          </button>
          {sortOpen&&<div className="sort-dropdown" role="menu" aria-label="Ordenar y filtrar libros">
            ...
            {/* Solo se ve en movil (lo apaga el CSS por encima de 760px). Alli los cuatro
                botones de categoria sueltos rompian la estetica, asi que se indexan aqui,
                en el mismo desplegable. `display:none` los saca tambien del tabulador. */}
            <div className="sort-dropdown-movil">
              <div className="sort-dropdown-divider" aria-hidden="true"/>
              <p className="sort-dropdown-label">Categoría</p>
              <button type="button" role="menuitemradio" aria-checked={categoryFilter===null} className={categoryFilter===null?"selected":""} onClick={()=>{setCategoryFilter(null);setSortOpen(false)}}>Todas las categorías{categoryFilter===null&&<span aria-hidden="true">✓</span>}</button>
              {categories.map(category=><button key={category} type="button" role="menuitemradio" aria-checked={categoryFilter===category} className={categoryFilter===category?"selected":""} onClick={()=>{setCategoryFilter(category);setSortOpen(false)}}>{category}{categoryFilter===category&&<span aria-hidden="true">✓</span>}</button>)}
            </div>
          </div>}
        </div>
